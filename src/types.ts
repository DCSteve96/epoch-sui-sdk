...
export interface AnalyticsData {
  ...
  monthlyBars:    Array<{ month: string; single: number; multi: number }>
  /** Monthly claim counts (aligned to `monthlyBars`) */
  claimBars:      Array<{ month: string; count: number }>
  /** Top 6 creators sorted by vault count */
  topCreators:    Array<{ address: string; count: number; pct: number }>
}
